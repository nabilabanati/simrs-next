...
export default function AdminPoliPage() {
    ...
    useEffect(() => {
        ...
        console.log('[Poli Page] Auth check:', { token: !!token, user });

        if (!token || !user) {
            console.log('[Poli Page] No token or user, redirecting to login');
            router.push("/login");
            return;
        }

        const userData = JSON.parse(user);
        console.log('[Poli Page] User data:', userData);
        
        if (userData.role?.toLowerCase() !== "superadmin") {
            console.log('[Poli Page] Not superadmin, role:', userData.role);
            router.push("/login");
            return;
        }

        console.log('[Poli Page] Auth passed, fetching polis');
        fetchPolis();
    }, [router]);
