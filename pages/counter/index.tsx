...
export default function AdminCounterPage() {
  ...
  useEffect(() => {
    const checkAuth = () => {
      try {
        ...
        if (!userStr) {
          console.log('❌ [Counter Dashboard] No user in localStorage, redirecting to login');
          router.push('/login?redirect=/counter');
          return;
        }

        const user = JSON.parse(userStr);
        
        // Only admin_loket can access counter dashboard
        if (user.role !== 'admin_loket') {
          console.log('❌ [Counter Dashboard] Invalid role, redirecting to unauthorized');
          router.push('/unauthorized');
          return;
        }

        console.log('✅ [Counter Dashboard] Auth check passed');
  ...
  // Apply filters - using mapped field names from API responses
