...
export default function PrintTicketPage() {
  ...
  const [loketStatus, setLoketStatus] = useState<Record<number, LoketStatus>>({
    1: { queueCount: 0 },
    2: { queueCount: 0 },
    3: { queueCount: 0 },
    4: { queueCount: 0 },
  ...
  const [lastAssignedLoket, setLastAssignedLoket] = useState<number | null>(null);
  const [currentTime, setCurrentTime] = useState<Date>(new Date());
  const [showModal, setShowModal] = useState(false);
  const [assignedLoket, setAssignedLoket] = useState<number>(1);
  ...
  const getOptimalLoket = (): number => {
    const lokets = Object.keys(loketStatus).map((loketId) => ({
      id: parseInt(loketId),
      queueCount: loketStatus[parseInt(loketId)].queueCount,
    }));

    let selectedLoket = lokets.reduce((min, current) =>
      current.queueCount < min.queueCount ? current : min
    );

    const minQueueCount = selectedLoket.queueCount;
    const availableLokets = lokets.filter(
      (loket) => loket.queueCount === minQueueCount
    );

    if (availableLokets.length > 1) {
      if (lastAssignedLoket) {
        const nextIndex = availableLokets.findIndex(
          (loket) => loket.id > lastAssignedLoket
        );
        selectedLoket =
          nextIndex !== -1 ? availableLokets[nextIndex] : availableLokets[0];
      } else {
  ...
  const handlePrintTicket = () => {
    const loket = getOptimalLoket();
    setLoketStatus((prev) => ({
      ...prev,
      [loket]: { queueCount: prev[loket].queueCount + 1 },
    }));
    ...
    setQueueHistory((prev) => [...prev, queueData]);
    setCurrentQueue((prev) => prev + 1);
    setAssignedLoket(loket);
    setLastAssignedLoket(loket);
    setShowModal(true);
  };

  const displayNumber = String(currentQueue - 1).padStart(3, '0');
  const lastQueue = queueHistory[queueHistory.length - 1];
  const displayLoket = lastQueue ? lastQueue.loket : 1;
      ...
      <main className="container mx-auto px-6 py-10">
        ...
        <div className="max-w-4xl mx-auto ">
          <Button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-8 px-8 rounded-xl text-2xl shadow-lg hover:shadow-xl transition-all duration-300 h-20"
            size="lg"
          >
            CETAK ANTRIAN
          </Button>
            ...
            <div className="mb-4">
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Antrian saat ini: {loketStatus[assignedLoket].queueCount}
                </p>
              </div>
            </div>
          </div>
